...
class Propagator(nn.Module):
    ...
    def process(self, initImg, contentImg):
        ...
        if isinstance(contentImg, str):
            content = np.array(Image.open(contentImg).convert('RGB'))
        else:
            content = np.array(contentImg)

        if isinstance(initImg, str):
            B = np.array(Image.open(initImg).convert('RGB')).astype(np.float64) / 255
        else:
            B = np.asarray(initImg).astype(np.float64) / 255

        # Extract dimensions and crop B
        h1, w1, k = B.shape
        h = h1 - 4
        w = w1 - 4
        ...
        content = Image.fromarray(content).resize((w, h), Image.ANTIALIAS)
        ...
        # Compute Laplacian and solve the sparse system
